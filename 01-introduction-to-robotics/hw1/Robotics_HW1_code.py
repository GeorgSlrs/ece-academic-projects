...
# Initial position of the cylinder - lying along the x-axis
T_world_cylinder = np.eye(4)
T_world_cylinder[:3, :3] = R_y_neg90
T_world_cylinder[:3, 3] = [69.50, 9.50, 2.50]

# Manipulated transformation matrix for the box (no change)
T_manipulated_box = T_world_box.copy()

# Manipulated transformation matrix for the cylinder - rotated back to align with world 'z' axis
...
def draw_cylinder(ax, T, radius=2.5, height=25):
    # Always draw along the cylinder's local z-axis
    theta = np.linspace(0, 2 * np.pi, 50)
