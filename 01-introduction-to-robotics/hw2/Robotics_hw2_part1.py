...
def log_rotation(R):
    # Clamp the argument of arccos to avoid numerical issues
...
def log_pose(T):
    R = T[:3, :3]
    p = T[:3, 3].reshape((3,1))
    rt = log_rotation(R)  # (3,1)
    theta = np.linalg.norm(rt)
    if np.allclose(theta, 0.):
        return np.block([[np.zeros((3,1))],
                         [p]])
    rh = hat(rt.flatten()/theta)
    A = (1./theta)*np.eye(3) - 0.5*rh + (1./theta - 0.5/np.tan(theta/2.))*(rh@rh)
    return np.block([[rt],
                     [A @ p * theta]])
...
for iteration in range(max_iterations):
    ...
    V_w = matrix_log6(T_err)  # 6x1 vector
    ...
    J_w = jacobian_space(SList, q_k)  # 6xn matrix
