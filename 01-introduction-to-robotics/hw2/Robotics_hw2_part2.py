...
for iteration in range(max_iterations):
    ...
    delta_q = (J_w_pinv @ V_w).ravel()  # Ensure delta_q is (7,)
    q_k = q_k + a * delta_q
...
def draw_box_with_hole(ax, T, width=20, height=10, depth=40, hole_radius=5):
...
def draw_cylinder(ax, T, radius=2.5, height=25, color='orange'):
    ...
    x_t = transformed_points[0,:].reshape(theta_grid.shape)
    y_t = transformed_points[1,:].reshape(theta_grid.shape)
    z_t = transformed_points[2,:].reshape(theta_grid.shape)
...
def draw_robot(ax, transformations, color='grey'):
    for i, T in enumerate(transformations):
        ...
        if i == len(transformations)-1:
            draw_frame(ax, T, label='End Effector')
        else:
            draw_frame(ax, T, label=f'J{i}')
...
# Draw box with hole (unchanged dimensions)
...
# Place the cylinder vertically above the hole
# We'll just place the cylinder at the final desired position but keep it vertical
T_world_cylinder = np.eye(4)
T_world_cylinder[:3, 3] = T_wd[:3, 3]  # Same position as T_wd, but no rotation so it's vertical
draw_frame(ax, T_world_cylinder, "Cylinder")
draw_cylinder(ax, T_world_cylinder, radius=2.5, height=25)
