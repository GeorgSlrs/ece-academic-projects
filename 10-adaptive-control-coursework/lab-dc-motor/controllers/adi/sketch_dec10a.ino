const int M1 = 7;  // Motor terminal 1 //for us 6
const int M2 = 5;  // Motor terminal 2 //for us 7

// Speed feedback pins
const int S1 = 2;  // Speed sensor (forward)//for us 4
...
float bm = 0.2;
float gammaVal = 0.02;  // Adaptation gain (used for both a_hat and b_hat)
float u = 0.0;

// Variables
float yp = 0.0;   // Plant output (internal model)
float ym = 0.0;   // Reference model output
float theta1 = 0.0, theta2 = 0.0; // Adaptive parameters: theta1 ~ a_hat, theta2 ~ b_hat
...
void loop() {
  ...
  if (t - lastTime * 0.001 >= dt) {
    ...
    // ---------------------------------------------------
    // ADI CONTROL LAW (Dynamic Inversion), discrete-time
    // theta1 ≈ a_hat, theta2 ≈ b_hat, y ≈ v1Filt
    // u = ((am - a_hat)*y + bm*r) / b_hat
    // ---------------------------------------------------
    float b_hat_safe = theta2;
    if (b_hat_safe > -0.01 && b_hat_safe < 0.01) {
      b_hat_safe = (b_hat_safe >= 0 ? 0.01 : -0.01);  // avoid division by zero
    }

    u = ((am - theta1) * v1Filt + bm * r) / b_hat_safe;
    // ---------------------------------------------------

    // Plant dynamics (Euler integration) - internal model only
    float dyp = -a * yp + b * u;
    yp += dyp * dt;

    // Reference model dynamics (same as MATLAB y_m_dot = a_m*y_m + b_m*r)
    float dym = -am * ym + bm * r;
    ym += dym * dt;

    // Tracking error (plant output - model output)
    e0 = v1Filt - ym;
    ...
    // a_hat_dot = gamma * y * e
    // b_hat_dot = gamma * u * e
    // here: theta1_dot = gammaVal * v1Filt * e0
    //       theta2_dot = gammaVal * u      * e0
    // ---------------------------------------------------
    float dtheta1 =  gammaVal * v1Filt * e0;
    float dtheta2 =  gammaVal * u      * e0;
    ...
    // PWM command (simple saturation 0..250)
    targetPWM = min(128+u, 250);
    Serial.println(u);
    Serial.println(targetPWM);
    Serial.println(v1Filt);
  }
