...
def prepare_data():
    ...
    print('Training samples per class:', np.unique(y_train, return_counts=True))
    print('Testing samples per class:', np.unique(y_test, return_counts=True))
...
def compute_loss(A2, Y, method='cross_entropy'):
    ...
    if method == 'cross_entropy':
        ...
        Y_mapped = 2 * Y - 1  # Map 0 -> -1, 1 -> 1
        loss = (1 / m) * np.sum(np.exp(-Y_mapped * A2))
    else:
...
def backward_pass(cache, Y, weights_2, method='cross_entropy'):
    ...
    if method == 'cross_entropy':
        ...
        Y_mapped = 2 * Y - 1  # Map 0 -> -1, 1 -> 1
        dZ2 = -Y_mapped * np.exp(-Y_mapped * A2)
    else:
        raise ValueError("Invalid method specified.")
...
# ----------------------------
# 5. Training Loop without Plotting (Plotting will be done in main script)
# ----------------------------
...
def train_network_adam(X_train, y_train, num_epochs=1000, learning_rate=0.001, lambda_=0.999, method='cross_entropy'):
    ...
    return weights_1, biases_1, weights_2, biases_2, losses
...
def evaluate(X, y, weights_1, biases_1, weights_2, biases_2, method='cross_entropy'):
    ...
    if method == 'cross_entropy':
        ...
        predictions = (cache['A2'] > 0).astype(int)
    error_rate = np.mean(predictions != y.reshape(-1, 1))
    return error_rate, predictions.flatten()
...
if __name__ == "__main__":
    ...
    # Train and collect losses for Cross-Entropy Method
    print("\nTraining with Cross-Entropy Method:")
    w1_ce, b1_ce, w2_ce, b2_ce, losses_ce = train_network_adam(X_train, y_train, method='cross_entropy')

    # Train and collect losses for Exponential Method
    print("\nTraining with Exponential Method:")
    w1_exp, b1_exp, w2_exp, b2_exp, losses_exp = train_network_adam(X_train, y_train, method='exponential')

    # Plot learning curves for both methods on the same graph
    plt.figure(figsize=(10, 6))
    plt.plot(losses_ce, label='Cross-Entropy Loss')
    plt.plot(losses_exp, label='Exponential Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Learning Curves for Cross-Entropy and Exponential Methods')
    plt.legend()
    plt.grid(True)
    plt.show()
    
    # Evaluate on test set for both methods
    error_rate_ce, predictions_ce = evaluate(X_test, y_test, w1_ce, b1_ce, w2_ce, b2_ce, method='cross_entropy')
    error_rate_exp, predictions_exp = evaluate(X_test, y_test, w1_exp, b1_exp, w2_exp, b2_exp, method='exponential')

    print(f"Cross-Entropy Test Error Rate: {error_rate_ce * 100:.2f}%")
    print(f"Exponential Test Error Rate: {error_rate_exp * 100:.2f}%")

    # Create a table of error rates per numeral
    def error_per_class(y_true, predictions):
        classes = np.unique(y_true)
        error_rates = {}
        for cls in classes:
            idx = np.where(y_true == cls)
            error = np.mean(predictions[idx] != y_true[idx])
            error_rates[cls] = error * 100
        return error_rates

    # Map labels back to numerals
    label_to_numeral = {0: 0, 1: 8}

    # Error rates per numeral for Cross-Entropy Method
    error_rates_ce = error_per_class(y_test, predictions_ce)
    print("\nError Rates per Numeral for Cross-Entropy Method:")
    for label, error_rate in error_rates_ce.items():
        numeral = label_to_numeral[label]
        print(f"Numeral {numeral}: {error_rate:.2f}%")
    print(f"Total Error Rate: {error_rate_ce * 100:.2f}%")

    # Error rates per numeral for Exponential Method
    error_rates_exp = error_per_class(y_test, predictions_exp)
    print("\nError Rates per Numeral for Exponential Method:")
    for label, error_rate in error_rates_exp.items():
        numeral = label_to_numeral[label]
        print(f"Numeral {numeral}: {error_rate:.2f}%")
    print(f"Total Error Rate: {error_rate_exp * 100:.2f}%")
